 ...
 * adjudicator is NOT. Gemini proposes a candidate chain of recipes toward a
 * named target; every proposal lands in a staging file and stays there until
 * a human applies the verification rubric (real transformation? does the
 * source say what we claim? right unit?) and moves it into gameData.ts by
 * hand. Nothing in src/ ever imports from data/staging.
 */
...
if (!target) {
  console.error('Usage: npx tsx scripts/propose.ts "<target name>"')
  process.exit(1)
}
...
For each recipe in the chain from raw/existing materials to the target, provide:
- output_id: snake_case id
- output_name: display name
- inputs: exactly two element ids (existing ids above, or new ones you introduce earlier in your own chain)
...
async function main() {
  ...
  console.log(`Wrote ${parsed.proposals.length} candidate recipe(s) to ${outPath.pathname}`)
  ...
  for (const p of parsed.proposals) {
    console.log(`\n  ${p.inputs.join(' + ')} -> ${p.output_name}  [${p.process}]`)
