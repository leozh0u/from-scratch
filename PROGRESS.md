...
| 2 | *"we are talking teaching how everything is made"* — get to Little Alchemy scale | **open, and it is the main remaining job.** 72 elements against Little Alchemy 2's 720. `SCALING.md` measures the three bottlenecks and what removing them costs: sprite vocabulary (~2h, removes the hard cap), Referenced citation tier with a link checker (~1h), probe-and-propose import pipeline (~2h). After those, review runs at 60-100 elements an hour. | Build the three unlocks, then author. My recommendation is 200-250 and stop at 02:00 so the video gets made; Leo has not picked a target. |
...
| 2c | Combining more than two at once | **open — needs Leo's call.** Little Alchemy is strictly two in both games; verified. Three inputs takes the unanswerable space from 2,554 pairs to ~62,000 triples, which the rule table cannot explain, and chains name the intermediate instead (raw meal, slaked lime). About an hour to build if he wants it anyway. |
