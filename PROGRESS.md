...
| 2b | *"add some fun small animations like the forrest"* for the city | **done.** Window lights found in the artwork itself and birds crossing the sky; `scripts/citytest.ts` measures both rates. Verified in-browser: 36 to 90 overlay pixels varying over ten seconds. |
| 3 | *"we want things that cant go together also explained why"* | **done.** 36 rules; the generic fallback went from 73% of the 1,713 non-recipe pairs to 13.3%, with 37 distinct answers. `explaintest.ts` pins it under a fifth. |
...
| *"we want things that cant go together also explained why"* | Measured across all 1,713 non-recipe pairs: fallback share 73% before, 13.3% after, 37 distinct answers, no line using the forbidden "impossible / cannot / never". |
