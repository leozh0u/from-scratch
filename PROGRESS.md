...
| 14 | *"...unlock every possible thing"*, targets replaced with things-missing, hint mode, 3 per realm then 1 per 10 found | **half done.** Hints are built exactly as asked: the SOLVER picks (never the model — a hint is a claim about the recipe graph, which is the one thing the runtime model may not know), it names one input and what kind of thing comes out, and pressing again on the same board spends a second hint and names both. Budget persists per realm as spend, not balance. The bench now carries a made-count. **Still to do: replace the three-target bar with what is missing.** | Swap TargetList for a missing view. |
| ~~15~~ | *"what should be on the left of the combine"* / *"should we have give up on the left"* | **answered and built: the made-count, not give up.** There is no fail state here to give up from, and deleting somebody's discoveries because they asked for help is a strange thing to do. What a stuck player wants is more help, so the hint escalates instead: press again and it names both inputs. Two hints for a solve, no reset. |
